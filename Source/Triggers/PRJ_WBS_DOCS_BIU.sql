--------------------------------------------------------
--  DDL for Trigger PRJ_WBS_DOCS_BIU
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE TRIGGER "PRJ"."PRJ_WBS_DOCS_BIU" 
...
begin
    if :new.wbc_id is null then
        :new.wbc_id := to_number(sys_guid(), 'XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX');
    end if;
    if inserting then
...
end prj_wbs_docs_biu;

/
ALTER TRIGGER "PRJ"."PRJ_WBS_DOCS_BIU" ENABLE;
